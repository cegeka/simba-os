...
| simba-ri | The _Reference Implementation_, this implies you can create your own implementation, and use the `api` and `core` components as an _SPI_. This component contains example database creation scripts which you'll need to have a working Simba implementation. So this should contain configuration of simba users etc. This specifically secures the simba-zoo application. | :white_check_mark: |
...
| simba-zoo | Just a dummy web application that is secured via the `simba-ri` component. | :white_check_mark: |
